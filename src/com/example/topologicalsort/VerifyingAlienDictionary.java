...
public class VerifyingAlienDictionary {
    ...
    // O(w * m) time w = length of words and m = length of largest word
    // O(1) space
    private static boolean verifyAlienDictionary(String[] words, String order) {
        if (words.length == 1) {
            return true;
        }
            ...
            for (int j = 0; j < words[i].length(); j++) {
                ...
                if (words[i].charAt(j) != words[i + 1].charAt(j)) {
                    if (orderMap.get(words[i].charAt(j)) > orderMap.get(words[i + 1].charAt(j))) {
                        ...
                        // No need to check rest if the initial character is sorted
                        // (age < eaf)
                        break;
                    }
                }
